...
public class ContentAwareImageResizing {
    ...
    public int[] leastContrastImageVPath() {
        // TODO
        WeightedDigraph weightedDigraph = this.makeVGraph();
        ShortestPathsTopological shortestPathsTopological = new ShortestPathsTopological(weightedDigraph, this.sx * this.sy);
        Stack<Integer> shortestPathsTopologicalPathTo = shortestPathsTopological.pathTo(this.sx * this.sy + 1);
        int[] leastContrastImageVPath = new int[shortestPathsTopologicalPathTo.size() - 2]; // first and last node --> -2
        shortestPathsTopologicalPathTo.pop(); // delete first (x * y)
        for (int i = 0; i < leastContrastImageVPath.length; i++) {
            leastContrastImageVPath[i] = shortestPathsTopologicalPathTo.pop() - (this.sx * i);
        }
        shortestPathsTopologicalPathTo.pop(); // delete last (x * y + 1)
        return leastContrastImageVPath;
    }
