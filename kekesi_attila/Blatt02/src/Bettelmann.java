...
public class Bettelmann {
    ...
    public void playRound() {
        ...
        Card card1;
        Card card2;
        Queue<Card> openedPile1 = new LinkedList<>(); // queue (FIFO)
        Queue<Card> openedPile2 = new LinkedList<>(); // queue (FIFO)
        int compareTo = 0;

        // play round
        while (compareTo == 0 && !this.closedPile1.isEmpty() && !this.closedPile2.isEmpty()) {
            card1 = this.closedPile1.pop(); // pop(): last of stack (FILO)
            card2 = this.closedPile2.pop(); // pop(): last of stack (FILO)
            openedPile1.add(card1);
            openedPile2.add(card2);
            compareTo = card1.compareTo(card2);
        }

        // player1 takes the piles
        if (compareTo > 0) {
            while (!openedPile1.isEmpty()) {
                this.closedPile1.add(openedPile1.poll()); // poll(): first of queue (FIFO)
            }
            while (!openedPile2.isEmpty()) {
                this.closedPile1.add(openedPile2.poll()); // poll(): first of queue (FIFO)
            }
        }

        // player2 takes the piles
        if (compareTo < 0) {
            while (!openedPile2.isEmpty()) {
                this.closedPile2.add(openedPile2.poll()); // poll(): first of queue (FIFO)
            }
            while (!openedPile1.isEmpty()) {
                this.closedPile2.add(openedPile1.poll()); // poll(): first of queue (FIFO)
            }
        }
